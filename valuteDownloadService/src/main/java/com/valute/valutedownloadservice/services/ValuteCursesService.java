...
//        }
    ...
    //Class for thread pool and ExecutorService
    @Data
    @AllArgsConstructor
        ...
        if (cDiff.get("days") < processors){
            ...
            try (ExecutorService executorService = Executors.newFixedThreadPool(8)) {
                ...
                while(partC.before(endC)){
                    partC.add(Calendar.YEAR, cDiff.get("years") / processors);
                    partC.add(Calendar.MONTH, cDiff.get("months") / processors);
                    partC.add(Calendar.DAY_OF_MONTH, cDiff.get("days") / processors);
...
//                    && cDiff.get("years") == 0)) {
//                return downloadingValuteCursesFunc(startDate, endDate);
//            } else {
